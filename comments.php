...
<p class="comment_meta">Subscribe to comments with <?php comments_rss_link(__('RSS')); ?> 
<?php if ( pings_open() ) : ?>
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<li id="comment-<?php comment_ID() ?>" class="<?php echo tj_comment_class() ?>">
			<div class="comment_author">
				...
				<strong><?php comment_author_link() ?></strong> said, on <?php comment_date('F jS, Y') ?> at <a href="<?php comment_link() ?>"><?php comment_time() ?></a> <? edit_comment_link(__('(Edit)', 'sandbox'), ' ', ''); ?>
			</div>
			<?php if ($comment->comment_approved == '0') : ?>
			<p class="commetn_mod"><em>Your comment is awaiting moderation.</em></p>
			<?php endif; ?>
